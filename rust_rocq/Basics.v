...
(** We use underscore (_) here as a _wildcard pattern_ to avoid
...
Proof.
  ...
  - intros H. 
    ...
    reflexivity. 
Qed.
(* Could be shorter with double [] [] intro. 
   This is a thinking solution *)
(** [] *)
...
(* FILL IN HERE 
  Fixpoint reject (n m : nat) : nat :=
    match n, m with 
    | O, _ => O
    | S n', O => O
    | S n', S m' => reject (S n') O
    end. 
...
(** **** Exercise: 3 stars, standard, optional (andb_eq_orb)
    ...
    not the best way.) *)
Lemma xandxeqx : forall x : bool, x && x = x.
Proof. 
  intros [].
  - reflexivity. 
  - reflexivity.
...
Lemma yoryeqy : forall y : bool, y || y = y.
Proof. 
  intros [].
  - reflexivity. 
  - reflexivity.
...
Definition lower_grade (g : grade) : grade :=
  match g with 
  | Grade F Minus => Grade F Minus (* hax *)
  | Grade gr m => 
  (match m with 
    | Plus => Grade gr Natural
    | Natural => Grade gr Minus
    | Minus => Grade (lower_letter gr) Plus  
    end)
end. 
